...
Live SCADA log simulator with level filtering and real event messages
Expandable accordion lessons with inline quizzes
Tap-to-check commissioning checklists (DC, inverter, AC/protection)
Full SCADA architecture diagram with protocol reference tables and Modbus register map
PWA features: Service worker for offline caching, installable manifest, dark/light mode toggle, WCAG AAA contrast, 44px minimum tap targets, 8px grid spacing, custom select arrows, nav active state with top border glow + icon background highlight.
What it does well:
Real engineering content (SRF-PLL math, Modbus framing, IDMT curves, Dyn11 explanation)
Structured modules with expandable lessons
Interactive quizzes with feedback
Working calculators with correct formulas
SCADA log simulation
